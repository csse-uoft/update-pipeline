...
listing1_onto, output_owl_filename = init('listing2')

# Load CSV
print(chalk.green("Loading CSV..."))
data = read_csv('./input/Final new Cluster id forQApart (Oct,5).csv')
listingId2Row = {}
for row in data:
...
with listing1_onto:
    for listingId in tqdm(listingId2Row.keys()):
        ...
        for (organization_or_program,) in result:
            # Add search words
            cp.hasSearchWords[organization_or_program] = [listingId2Row[listingId]["Categories"]]
            # Add Localities
            cp.hasLocality[organization_or_program] = [listingId2Row[listingId]["Localities"]]
            # Remove letter in listing ID
            cp.hasID[organization_or_program] = [re.sub(r"[a-zA-Z]", "", listingId)]
